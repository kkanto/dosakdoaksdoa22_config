...
def job_config = [
        job: [
                name: "dosakdoaksdoa22_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'dosakdoaksdoa22_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/dosakdoaksdoa22_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.dosakdoaksdoa22.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "dosakdoaksdoa22", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("dosakdoaksdoa22_Pipeline_develop");

job.with {

    authenticationToken('2r5Y7FOtQNpqzrkvFZnb8h5rmPHJEQHp')
